...
int main(int argc, char** argv)
{
	...
	sdds::Collection<sdds::Book> library("Bestsellers");
	{
		...
		do {
			...
			if (file) {
				if (fileLine[0] != '#') {// Check if this is a commented line.
					library += Book(fileLine);
			//		debug++;
				}
			}
		} while (file && library.size() < 4); //load the first 4 books
		//^^^^^^^^^^ my work ^^^^^^^^^^//
	...
	//////////// my work ////////////
	auto fixPrice = [&](Book& book) //single parameter of type "Book&"
	{
		if (book.country() == "US") //in US, multiply the price with "usdToCadRate"
		{
			double* price = &book.price();
			*price *= usdToCadRate;
		}
		else if (book.country() == "UK" && book.year() >= 1990 && book.year() <= 1999) //in UK between 1990 and 1999 (inclussive)
			{
				double* price = &book.price();
				*price *= gbpToCadRate;
			}
	...
	//////////// my work //////////// //updated
	//for (auto& book : library) //does not work anymore
	...
	Movie movies[5];
	{
		...
		do {
			...
			// if the previous operation failed, the "file" object is in error mode
			if (file) {
				if (fileLine[0] != '#') {// Check if it is a commented line.
					movies[count] = Movie(fileLine);
					count++;
				}
			}
		} while (file);
